      ...
      {isAdmin ? (
        <div className="tool-grid">
          <section className="tool-panel">
            <h3>学生成绩信息</h3>
            <p>
              支持 .xlsx 或 .csv，表头为：学生姓名、成绩、老师姓名、班级类型、课线、战区、作业次数、视频次数、学生消息数。课线填写 Python、探月、小火箭或幼儿，用于匹配专属规划、规划明细和时间表物料；Python 课线填写英才班时调用 Python 英才班物料，幼儿课线班级类型填写英才班。综合得分由系统自动生成；旧表未填写课线时默认使用探月。
            </p>
      ...
      <section className="tool-panel wide">
        ...
        <form className="single-student-form" onSubmit={addSingleStudent}>
          ...
          <label>
            <span>课线</span>
            <select
              value={newStudentCourseLine}
              onChange={(event) => {
                const courseLine = event.target.value as CoursePlanLineId;
                setNewStudentCourseLine(courseLine);
                if (courseLine === "preschool") setNewStudentProgramType("英才班");
              }}
            >
